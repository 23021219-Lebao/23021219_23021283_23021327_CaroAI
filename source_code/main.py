...
class CaroGUI:
    def __init__(self, root):
        """Khởi tạo giao diện và các đối tượng logic game"""
        self.root = root
        ...
        # --- Khu vực điều khiển (Thuật toán, Độ sâu, Nút chơi lại) ---
        ...
        # Nhãn hiển thị trạng thái và thông số AI
        self.stats_label = tk.Label(root, text="Sẵn sàng! Mời bạn (X) đi trước.", font=('Arial', 11), fg="blue")
        self.stats_label.pack(pady=10)

    def update_ui(self):
        """Cập nhật trạng thái hiển thị (X, O hoặc ô trống) lên lưới nút bấm"""
        for r in range(self.size):
            for c in range(self.size):
    ...
    def player_click(self, r, c):
        """Xử lý sự kiện khi người chơi click chọn ô cờ"""
        if self.game_over or self.board.board[r][c] != 0:
            return
            
        self.board.make_move(r, c, 1) # Người chơi (1) đánh
        self.update_ui()
        ...
        # Hoãn 100ms trước khi AI chạy để giao diện kịp cập nhật chữ "đang suy nghĩ"
        self.root.after(100, self.ai_turn)

    def ai_turn(self):
        """Kích hoạt thuật toán AI tính toán và thực hiện nước đi"""
        ...
        # Gọi thuật toán tương ứng được chọn trên UI
        if use_ab:
        ...
        if best_move:
            self.board.make_move(best_move[0], best_move[1], 2) # AI (2) đánh
            self.update_ui()
            
            # Hiển thị thông số hiệu năng của AI lên màn hình
    ...
    def handle_game_over(self, winner):
        """Thông báo kết quả khi trận đấu kết thúc"""
        self.game_over = True
        if winner == 1:
    ...
    def reset_game(self):
        """Làm mới toàn bộ bàn cờ để bắt đầu ván mới"""
        self.board = CaroBoard(self.size)
        self.game_over = False
